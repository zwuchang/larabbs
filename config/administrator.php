...
return array(
     ...
     * 数组里的值有三种类型：
     * 1.字符串 --- 子菜单的入口，不可访问；
     * 2.模型配置文件 --- 访问 `model_config_path` 目录下的模型文件，如'users'访问的是'user.php'模型配置文件；
     ...
     * 权限控制的回调函数。
     *
     * 此回调函数需要返回true或false,用来检测当前用户是否有权限访问后台。
     * 'true'为通过，'false'会将页面重定向到'login_path'选项定义的URL中。
     *
     ...
     * 使用布尔值来设定是否使用后台主页面。
     *
     * 如值为true,将使用'dashboad_view'定义的视图文件渲染页面；
     * 如值为false,将使用'home_page'定义的菜单条目来作为后台主页。
     *
    ...
    /*
     * 当选项permission权限检测不通过时，会重定向用户到此处设置的路径
